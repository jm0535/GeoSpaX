...
async function sampleBand(
  ...
  if (!shell.app.readRasterWindow) throw new Error("This GeoLibre host does not expose raster-window reads.");
  const reading = await shell.app.readRasterWindow(layerId, { bounds, width, height, band });
  if (!reading) throw new Error("The selected layer could not be sampled as a raster in the current view.");
...
export function mountIndexTool(
  ...
  const card = shell.addTool(parent, {
    ...
    method: "View-dependent aligned raster-window sampling; Otsu is offered as a suggestion, never silently substituted for the selected threshold.",
  });
  ...
  const reportState = () => defaults.onStateChange?.({
  ...
  for (const control of [raster.select, bandA, bandB, threshold]) control.addEventListener("change", reportState);
  updatePreset();
  card.append(
    field("Raster layer", raster.select),
    field("Index preset", preset),
    presetHint,
    fieldGrid(field("Band A", bandA), field("Band B", bandB), field("Extent threshold (≥)", threshold)),
  );
  ...
  const compute = async () => {
    ...
    const [a, b] = await Promise.all([
      sampleBand(shell, raster.select.value, bandAValue, context.bounds, context.width, context.height),
      sampleBand(shell, raster.select.value, bandBValue, context.bounds, context.width, context.height),
    ]);
    ...
    if (!grid.validCells) throw new Error("The sampled bands contain no valid normalized-difference cells.");
  ...
  suggest.addEventListener("click", () => {
    void withBusy(suggest, status, "Sampling bands and estimating Otsu threshold…", async () => {
      ...
      if (data.otsu.threshold === null) {
        setStatus(status, "warning", "Otsu is unavailable because the sampled histogram has fewer than two occupied classes.");
      } else {
        threshold.value = data.otsu.threshold.toFixed(4);
        setStatus(status, "success", `Otsu suggests ${threshold.value}; review it before creating an extent.`);
      }
      const stats = indexStatsRow(data.grid);
      renderKeyValueTable(results, [
        ["Valid / no-data cells", `${stats.validCells.toLocaleString()} / ${stats.nodataCells.toLocaleString()}`],
        ["Range", `${formatNumber(stats.min, 4)} to ${formatNumber(stats.max, 4)}`],
        ["Mean", formatNumber(stats.mean, 4)],
        ["Otsu suggestion", data.otsu.threshold === null ? "Unavailable" : formatNumber(data.otsu.threshold, 4)],
  ...
  run.addEventListener("click", () => {
    void withBusy(run, status, "Sampling, classifying and polygonizing the index…", async () => {
      ...
      const outputId = addOutputLayer(shell, `${presetId} extent ≥ ${selectedThreshold}`, outputFeatures, provenance);
      ...
      const warning = pix
        ? rasterResolutionWarning(pix.pixelWidthM, pix.pixelHeightM, data.grid.width, data.grid.height)
        : null;
      setStatus(status, "success", `Created ${presetId} extent from ${polygons.length.toLocaleString()} sampled cell(s).`);
      renderKeyValueTable(results, [
        ["Index", presetId],
      ...
      shell.recordRun(runRecord("normalized-difference-index", `${presetId} extent`, provenance, [outputId], {
        preset: presetId,
        threshold: selectedThreshold,
...
export function mountSlopeTool(
  ...
  const options = typeof subjectOrOptions === "string" ? { subject: subjectOrOptions } : subjectOrOptions;
  ...
  const card = shell.addTool(parent, {
    ...
    method: "Horn 3×3 derivative; border and no-data neighbourhoods are excluded. Pixel metres are computed at the viewport centre latitude.",
  });
  const raster = layerPicker(shell, {
    kind: "raster",
  ...
  const reportState = () => options.onStateChange?.({
    rasterLayerName: raster.layer()?.name ?? null,
    band: Number(band.value),
    break1: Number(break1.value),
    break2: Number(break2.value),
  });
  for (const control of [raster.select, band, break1, break2]) control.addEventListener("change", reportState);
  card.append(
  ...
  run.addEventListener("click", () => {
    void withBusy(run, status, "Sampling DEM and deriving Horn slope…", async () => {
      ...
      const context = samplingContext(shell);
      const sample = await sampleBand(shell, raster.select.value, parseFinite(band, "Band"), context.bounds, context.width, context.height);
      const pixel = pixelSizeAtCentre(context.bounds, sample.reading.width, sample.reading.height);
      if (!pixel) throw new Error("Could not derive finite pixel ground dimensions for the current view.");
      ...
      if (!horn.validCells) throw new Error("No valid 3×3 DEM neighbourhoods were available in the sampled view.");
      const breaks: [number, number] = [parseFinite(break1, "Break 1"), parseFinite(break2, "Break 2")];
      const classes = classifySlope(horn.slopeDeg, breaks, horn);
      const mask = steepMask(horn.slopeDeg, breaks, 2);
      const polygons = maskToPixelPolygons(mask, sample.reading.width, sample.reading.height, context.bounds);
      const provenance = makeProvenance(
        "horn-slope-zones",
      ...
      if (polygons.length) {
        ...
        outputIds.push(addOutputLayer(shell, `Steep slope ≥ ${classes.breaks[1]}°`, outputFeatures, provenance));
      }
      const areas = classes.counts.map((count) => areaM2ForPixelCount(count, pixel.pixelAreaM2));
      setStatus(status, polygons.length ? "success" : "warning", polygons.length
      ...
      renderKeyValueTable(results, [
        [classes.labels[0], `${classes.counts[0].toLocaleString()} cells · ${formatArea(areas[0]).display}`],
        [classes.labels[1], `${classes.counts[1].toLocaleString()} cells · ${formatArea(areas[1]).display}`],
        [classes.labels[2], `${classes.counts[2].toLocaleString()} cells · ${formatArea(areas[2]).display}`],
        ["Valid cells", classes.totalValid],
        ["Border excluded", classes.borderExcluded],
        ["No-data neighbourhoods", classes.nodataExcluded],
        ["Pixel at view centre", `${formatNumber(pixel.pixelWidthM, 1)} × ${formatNumber(pixel.pixelHeightM, 1)} m`],
        ["Sample grid", `${sample.reading.width} × ${sample.reading.height}`],
      ], "Slope zones");
      const warning = terrainResolutionWarning(pixel.pixelWidthM, pixel.pixelHeightM, sample.reading.width, sample.reading.height);
      if (warning) appendNotice(results, warning, "warning");
      shell.recordRun(runRecord("horn-slope-zones", "Terrain slope zones", provenance, outputIds, {
...
function comparisonMask(values: number[], nodata: number | null, threshold: number, operator: ComparisonOperator): Uint8Array {
...
function dynamicHistogram(values: number[], nodata: number | null) {
  const valid = values.filter((value) => Number.isFinite(value) && (nodata === null || value !== nodata));
  if (!valid.length) throw new Error("The sampled raster band contains no valid cells.");
  let min = Math.min(...valid);
  let max = Math.max(...valid);
  if (min === max) throw new Error(`The sampled raster is constant (${min}); there is nothing to threshold.`);
  const array = Float32Array.from(values.map((value) =>
...
export function mountRasterReclassTool(shell: PanelShell, parent: HTMLElement, subject = "raster extent"): void {
  const card = shell.addTool(parent, {
    ...
    method: "Manual or Otsu-suggested threshold over a view-dependent raster window; output follows sampled cell edges and inherits their resolution.",
  });
  ...
  card.append(field("Raster layer", raster.select), fieldGrid(field("Band", band), field("Threshold", threshold), field("Comparison", operator)));
  const suggest = button("Suggest Otsu threshold", "secondary");
  ...
  const sample = async () => {
    if (!raster.select.value) throw new Error("Select a raster layer.");
    const context = samplingContext(shell);
    const sampled = await sampleBand(shell, raster.select.value, parseFinite(band, "Band"), context.bounds, context.width, context.height);
    return { context, sampled, distribution: dynamicHistogram(sampled.reading.values, sampled.reading.nodata) };
  };
  suggest.addEventListener("click", () => {
    void withBusy(suggest, status, "Sampling raster and estimating Otsu threshold…", async () => {
      ...
      if (otsu.threshold === null) {
        ...
        setStatus(status, "success", `Otsu suggests ${threshold.value}; review it before polygonizing.`);
      }
      renderKeyValueTable(results, [
        ["Valid cells", data.distribution.valid],
        ["Range", `${formatNumber(data.distribution.min, 4)} to ${formatNumber(data.distribution.max, 4)}`],
        ["Otsu suggestion", otsu.threshold === null ? "Unavailable" : formatNumber(otsu.threshold, 4)],
  ...
  run.addEventListener("click", () => {
    void withBusy(run, status, "Reclassifying sampled cells and polygonizing…", async () => {
      ...
      const mask = comparisonMask(data.sampled.reading.values, data.sampled.reading.nodata, thresholdValue, op);
      const cells = mask.reduce((sum, value) => sum + value, 0);
      if (!cells) throw new Error("No sampled cells pass the selected threshold.");
      const polygons = maskToPixelPolygons(mask, data.sampled.reading.width, data.sampled.reading.height, data.context.bounds);
      const provenance = makeProvenance(
      ...
      const outputFeatures = dissolvePixelPolygons(polygons, { class: 1, threshold: thresholdValue, operator: op });
      const outputId = addOutputLayer(shell, `${layerName(shell, raster.select.value)} ${op} ${thresholdValue}`, outputFeatures, provenance);
      const measured = measureArea(outputFeatures, "equalarea");
      const pixel = pixelSizeAtCentre(data.context.bounds, data.sampled.reading.width, data.sampled.reading.height);
      ...
      if (pixel) {
        const warning = rasterResolutionWarning(pixel.pixelWidthM, pixel.pixelHeightM, data.sampled.reading.width, data.sampled.reading.height);
        if (warning) appendNotice(results, warning, "warning");
      }
      appendNotice(results, "Polygon boundaries follow sampled pixel edges; quote the pixel size/sample grid with any area result.");
      shell.recordRun(runRecord("raster-reclassify-polygonize", "Raster reclassify & polygonize", provenance, [outputId], {
        passingCells: cells,
        threshold: thresholdValue,
        operator: op,
        areaM2: measured.m2,
...
export function mountRasterChangeTool(shell: PanelShell, parent: HTMLElement, subject = "cover"): void {
  const card = shell.addTool(parent, {
    id: "raster-change",
    ...
    method: "Difference = T1 − T2. Loss is difference ≥ threshold; gain is difference ≤ −threshold. Inputs are sampled to one shared grid.",
  });
  ...
  card.append(
    ...
    fieldGrid(field("T1 band", band1), field("T2 band", band2), field("Change magnitude threshold", threshold)),
  );
  ...
  const compute = async () => {
    ...
    const context = samplingContext(shell);
    const [a, b] = await Promise.all([
      sampleBand(shell, t1.select.value, parseFinite(band1, "T1 band"), context.bounds, context.width, context.height),
      sampleBand(shell, t2.select.value, parseFinite(band2, "T2 band"), context.bounds, context.width, context.height),
    ]);
    if (a.reading.width !== b.reading.width || a.reading.height !== b.reading.height) throw new Error("The two raster samples are not aligned.");
    const grid = differenceGrid(a.reading.values, b.reading.values, a.reading.width, a.reading.height, a.reading.nodata, b.reading.nodata);
    if (!grid.validCells) throw new Error("No aligned valid cells were available in both raster dates.");
  ...
  suggest.addEventListener("click", () => {
    void withBusy(suggest, status, "Sampling and inspecting the signed difference…", async () => {
      const data = await compute();
      const candidates = [data.otsu.otsuGain, data.otsu.otsuLoss === null ? null : Math.abs(data.otsu.otsuLoss)].filter((value): value is number => value !== null && value > 0);
      if (candidates.length) threshold.value = Math.max(...candidates).toFixed(4);
      setStatus(status, candidates.length ? "success" : "warning", candidates.length
      ...
      renderKeyValueTable(results, [
        ...
        ["Difference range", `${formatNumber(Math.min(...finite), 4)} to ${formatNumber(Math.max(...finite), 4)}`],
        ["Otsu loss side", data.otsu.otsuGain === null ? "Unavailable" : formatNumber(data.otsu.otsuGain, 4)],
        ["Otsu gain side", data.otsu.otsuLoss === null ? "Unavailable" : formatNumber(data.otsu.otsuLoss, 4)],
      ], "T1 − T2 difference");
    });
  });
  run.addEventListener("click", () => {
    void withBusy(run, status, "Differencing rasters and polygonizing change…", async () => {
      ...
      if (!lossCount && !gainCount) throw new Error("No aligned cells meet the selected positive/negative change threshold.");
      const lossPolygons = maskToPixelPolygons(lossMask, data.grid.width, data.grid.height, data.context.bounds);
      const gainPolygons = maskToPixelPolygons(gainMask, data.grid.width, data.grid.height, data.context.bounds);
      const provenance = makeProvenance(
      ...
      const outputId = addOutputLayer(shell, `${subject} raster change`, outputFeatures, provenance);
      ...
      setStatus(status, "success", `Mapped ${lossCount.toLocaleString()} loss and ${gainCount.toLocaleString()} gain cell(s).`);
      renderKeyValueTable(results, [
        ["Difference convention", "T1 − T2"],
        ["Magnitude threshold", formatNumber(magnitude, 4)],
        ["Loss cells / area", `${lossCount.toLocaleString()} / ${formatArea(lossArea.m2).display}`],
        ["Gain cells / area", `${gainCount.toLocaleString()} / ${formatArea(gainArea.m2).display}`],
        ["Valid / no-data cells", `${data.grid.validCells} / ${data.grid.nodataCells}`],
        ["Sample grid", `${data.grid.width} × ${data.grid.height}`],
      ], "Raster change result");
      appendNotice(results, "This is thresholded value change, not sensor calibration or atmospheric correction. Confirm both rasters are semantically comparable.", "warning");
      shell.recordRun(runRecord("raster-change-detection", "Raster change detection", provenance, [outputId], {
        threshold: magnitude,
        lossCells: lossCount,
        gainCells: gainCount,
