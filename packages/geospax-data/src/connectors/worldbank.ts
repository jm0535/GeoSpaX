...
export async function fetchWorldBankIndicator(country: string, indicator: string, opts:{from?:number; to?:number; signal?:AbortSignal}={}): Promise<{ url:string; citation: Citation; rows: unknown[] }> {
  ...
  if (!res.ok) throw new Error(`WorldBank ${res.status}: ${await res.text().catch(()=>res.statusText)}`);
