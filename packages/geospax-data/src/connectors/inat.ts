...
export async function fetchInatObservations(params: FetchParams & { taxon: string }): Promise<{ url: string; citation: Citation; geojson: GeoJSON.FeatureCollection }> {
  ...
  if (!res.ok) throw new Error(`iNat ${res.status}: ${await res.text().catch(()=>res.statusText)}`);
  const data = await res.json() as { results: Array<{ geojson?: { coordinates:[number,number]}; location?:string; taxon?:{name:string}; observed_on?:string }> };
  const features: GeoJSON.Feature[] = (data.results ?? []).map(r=>{
    let lon:number|null=null, lat:number|null=null;
    if (r.geojson?.coordinates) { [lon,lat]=r.geojson.coordinates; }
    else if (r.location) { const p=r.location.split(","); lat=Number(p[0]); lon=Number(p[1]); }
    if (!Number.isFinite(lon!) || !Number.isFinite(lat!)) return null;
    return { type:"Feature", geometry:{ type:"Point", coordinates:[lon!,lat!] }, properties:{ scientificName: r.taxon?.name ?? params.taxon, observed_on: r.observed_on ?? null, source:"iNaturalist" } } as GeoJSON.Feature;
